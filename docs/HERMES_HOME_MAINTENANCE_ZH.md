...
修复、重新部署、对比行为时，必须优先参考这个仓库。不要只去查原版 `EKKOLearnAI/hermes-web-ui`，否则会把本项目故意改掉的 profile selector 删除、网关选项卡、master/slave 网关管理等功能误判为异常。
...
??????????????????? WebUI???? `/usr/lib/node_modules/hermes-web-ui`??? `/opt/hermes-home` ??????? `hermes-web-ui.service`???? `8648` ?????
